...
unsigned char WaitForDTMF(unsigned long TimeOut,unsigned char DTMF,unsigned long DelayAfterGetDTMF){ 

  unsigned BreakWaiting=0;
...
*/
    if(WaitForDTMF(10000,3,StageTimeout)==0) return 0;    //10秒內等不到3就離開
    //=========================送QUEUE DATA STAGE2========================
...
*/
    //========================送完後從QUEUE刪除===========================      
    if(WaitForDTMF(10000,3,StageTimeout)==0){ //10秒內等不到3就失敗..若等到了就刪QUEUE
      return 0;
    }else{
...
//===================================新就板韌體差異處
    ...
    Delayms(500);
//===================================
    if(WaitForDTMF(20000,3,StageTimeout)==0) return 0;  //撥放TTS(系統設定完成)並等待3
...
unsigned char DataOut(unsigned char TelID){
  ...
  if(WaitForDTMF(1000*SystemConfig_ToneTimeout,3,StageTimeout)==0) return 0;
  if(DriverFlag.RDMode==0)EnableSpeaker(SystemConfig_SPK_DTMF);
  //==============================================送出MSG QUEUE中的資料
  ...
  if(WaitForDTMF(10000,3,0)==1){
    //==============================================播放"資料傳送完成"語音
    EnableSpeaker(SystemConfig_SPK_Voice);
    if(WaitForDTMF(20000,3,StageTimeout)==0){
      if(DriverFlag.RDMode==0)EnableSpeaker(SystemConfig_SPK_DTMF);
      return 0;
    }
    //==============================================取得待會是否要接受資料
